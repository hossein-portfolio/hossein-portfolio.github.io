...
fontLoader.load("./fonts/violet.json", (font) => {
    ...
    scene.add(textMesh)
    geometry = new THREE.TextGeometry("        Hossein", {
        font: font, 
        size: 0.06,
        height: 0.05
    })
...
let onClick = (event) => {
    ...
    if (intersects.length > 0 ) {
        ...
        if (!comp && intersects[0].object.name.startsWith("defaultMaterial")) {
            window.open('assets/bike.jpg', '_blank')
        }
        if (!comp && intersects[0].object.name.startsWith("Object_24")) {
            window.open('https://www.playstation.com/en-us/ps5/', '_blank')
        }
        if (!comp && intersects[0].object.name.startsWith("skate")) {
            window.open('assets/skate.jpg', '_blank')
        }
